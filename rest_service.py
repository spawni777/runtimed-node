...
from config import RUNTIMED_ASSETS_PATH, RUNTIMED_EXECUTABLE_PATH
from logger import logger
from runtimed import RuntimedConfig, RuntimedCore
...
class Service(object):
    def __init__(self):
        ...
        self.core = RuntimedCore(
            executable_path=RUNTIMED_EXECUTABLE_PATH,
            assets_path=RUNTIMED_ASSETS_PATH
        )
    ...
    def connect(self, request: Request):
        ...
        # Same client reconnecting without /disconnect (e.g. retries, duplicate HTTP
        # clients) must not stop the core and invalidate the session.
        if self.connected and peer_ip == self.client_ip:
            logger.debug(
                '%s duplicate POST /connect; keeping session %s',
                peer_ip,
                self.session_id,
            )
        ...
        if self.connected:
            logger.warning(
                'New connection from %s, core control taken from previous client (%s).',
                peer_ip,
                self.client_ip,
            )
        ...
        self.client_ip = peer_ip
        self.connected = True
        logger.info(f'{peer_ip} connected, Session ID = "{self.session_id}".')
    ...
    def start(self, session_id: UUID = Body(embed=True), config: str = Body(embed=True)):
        ...
        try:
            config = RuntimedConfig(config, self.client_ip)
        ...
        with self.core.get_logs() as logs:
            try:
                ...
                last_log = ''
                saw_started_line = False
                while time.time() < end_time:
                    while logs:
                        log = logs.popleft()
                        if log:
                            last_log = log
                        if self.core_version and (
                            f'Runtimed {self.core_version} started' in log
                        ):
