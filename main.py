...
from config import (RUNTIMED_API_PORT, SERVICE_HOST, SERVICE_PORT,
...
if __name__ == "__main__":
    ...
    if SERVICE_PROTOCOL == 'rpyc':
        ...
        thread = ThreadedServer(rpyc_service.RuntimedService(),
                                port=SERVICE_PORT,
        ...
        logger.info(f"Node service running on :{SERVICE_PORT}")
        logger.info(
            "Runtimed node: Port=%s, API port=%s. "
            "The core binds :%s only after the control panel calls /start.",
            SERVICE_PORT,
            RUNTIMED_API_PORT,
            RUNTIMED_API_PORT,
        )
