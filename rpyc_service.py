...
class RuntimedCoreLogsHandler(object):
    def __init__(self, core: RuntimedCore, callback: callable, interval: float = 0.6):
        self.core = core
...
@rpyc.service
class RuntimedService(rpyc.Service):
    def __init__(self):
        self.core = None
    ...
    def start(self, config: str):
        ...
        try:
            config = RuntimedConfig(config, self.connection.peer)
            self.core = RuntimedCore(executable_path=RUNTIMED_EXECUTABLE_PATH,
    ...
    @rpyc.exposed
    def fetch_runtimed_version(self):
        if self.core is None:
            raise ProcessLookupError("Runtimed has not been started")

        return self.core.version

    @rpyc.exposed
    def fetch_logs(self, callback: callable) -> RuntimedCoreLogsHandler:
        if self.core:
            logs = RuntimedCoreLogsHandler(self.core, callback)
